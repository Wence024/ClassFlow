import { describe, it, expect } from 'vitest';
import { buildPdfFilename } from '../pdfExportService';
import type { InstructorReport } from '../../types/instructorReport';

describe('pdfExportService', () => {
  it('uses expected filename format', () => {
    const report = buildReport();
    const filename = buildPdfFilename(report);
    expect(filename).toMatch(/^INS123_First_Semester_S\.Y\._2025-2026_Schedule\.pdf$/);
  });
});

function buildReport(): InstructorReport {
  return {
    instructor: {
      id: 'ins',
      code: 'INS123',
      first_name: 'Jane',
      last_name: 'Doe',
      prefix: null,
      suffix: null,
      department_id: 'dep', // required by Instructor type
      department_name: 'CS',
      email: 'jd@x.test',
      created_at: '',
    },
    semester: { id: 'sem', name: 'First Semester S.Y. 2025-2026' },
    schedules: { mondayWednesday: [], tuesdayThursday: [], friday: [], saturday: [] },
    totals: { lecHours: 0, labHours: 0, totalUnits: 0, totalLoad: 0 },
    loadStatus: 'AT_STANDARD',
  };
}


