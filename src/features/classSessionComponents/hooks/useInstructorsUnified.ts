...
export function useInstructorsUnified() {
  ...
  return {
    ...
    addInstructor: async (_data: InstructorInsert) => { throw new Error('Not authorized'); },
    updateInstructor: async (_id: string, _data: InstructorUpdate) => { throw new Error('Not authorized'); },
    removeInstructor: async (_id: string) => { throw new Error('Not authorized'); },
    isSubmitting: false,
    isRemoving: false,
    canManage: false,
  };
