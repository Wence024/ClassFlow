...
export function useClassroomsUnified() {
  ...
  return {
    ...
    addClassroom: async (_data: ClassroomInsert) => { throw new Error('Not authorized'); },
    updateClassroom: async (_id: string, _data: ClassroomUpdate) => { throw new Error('Not authorized'); },
    removeClassroom: async (_id: string) => { throw new Error('Not authorized'); },
    isSubmitting: false,
    isRemoving: false,
    canManage: false,
  };
