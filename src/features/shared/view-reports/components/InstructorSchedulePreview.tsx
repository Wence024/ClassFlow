 ...
 * @param root0.report The complete instructor report to preview.
 * @returns The rendered preview content.
 */
  ...
  return (
    <div className="bg-background p-8 print:p-0">
      ...
      <div className="mb-8 border-b pb-6">
        <h1 className="text-2xl font-bold text-center mb-6">INSTRUCTOR SCHEDULE REPORT</h1>
        ...
        <div className="grid grid-cols-2 gap-4 text-sm">
          <div>
            ...
            <p>
              ...
              {report.instructor.department_name || 'N/A'}
            </p>
            <p>
              <span className="font-semibold">Semester:</span> {report.semester.name}
            </p>
          </div>
      ...
      <div className="mt-8 border-t pt-6">
        ...
        <div className="grid grid-cols-2 gap-4 text-sm mb-8">
          <div className="space-y-2">
            <p>
              <span className="font-semibold">Total Lecture Hours:</span>{' '}
              {report.totals.lecHours.toFixed(1)}
            </p>
            <p>
              <span className="font-semibold">Total Lab Hours:</span>{' '}
              {report.totals.labHours.toFixed(1)}
            </p>
          </div>
          <div className="space-y-2">
            <p>
              <span className="font-semibold">Total Units:</span>{' '}
              {report.totals.totalUnits.toFixed(1)}
            </p>
            <p>
              <span className="font-semibold">Total Load:</span>{' '}
              {report.totals.totalLoad.toFixed(2)}
            </p>
            <p>
              <span className="font-semibold">Load Status:</span> {report.loadStatus}
            </p>
          </div>
        </div>
